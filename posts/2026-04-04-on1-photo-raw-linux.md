...
While Linux is the best environment for development purposes, Windows has been my goto for gaming and photo processing needs. But given how much time I spend in Linux, I naturally wonder whether I could reduce the need to dual boot. 

Gaming is certainly making an improvement thanks to the efforts of Proton and Wine, but photo processing has always felt out of reach. Recently though, the same effort that's gone into gaming has made it possible to run ON1 on Linux, and it works quite well. In this post I will go over the steps I took.  
...
The most common way to run Windows applications on Linux is through Wine, which is is a compatibility layer that emulates the Windows API and is sufficient for most applications. However, the most popular photo processing software, Lightroom, simply doesn't work well on Wine. Thankfully I recently switched to [ON1 Photo Raw](/posts/2024-05-05-moving-on-from-lightroom.md) which has been working well for what I need. In my searching I came across [a reddit thread](https://www.reddit.com/r/winehq/comments/1punxow/success_with_hiccups_on1_photo_raw_using/) discussing getting ON1 working with Wine, so I decided to give it a try and after a bit of trial and error I got it working.
...
The steps involved are to get the get Lutris to manage Wine, and run the ON1 installer and its dependencies through Lutris. 
...
Open Lutris, click the `+` button, and a dialog with install options appears. I went through the local install script way (third option) as well as the manual way (last option). 
...
The local install script way is simplest, it takes a YML file which describes the steps needed to get ON1 and its dependencies working. Save this file: 
...
Clicking next will then run the installer steps, which can take a while. During the installation, the Microsoft .NET installer will pop up, just click through the steps to complete it. 
...
When I then went in and installed .NET 4.8 manually through Lutris, the ON1 application launched properly. 

### Performance

It might just be because I'm doing light testing but the performance feels really fast, and I'm not sure why. It's not like I'm just browsing either, I'm making it go through masking layers, using some of the generative features, including erase, sky replacements, etc. It feels quite fast.
