...
Just placing this file at `.devcontainer/devcontainer.json` in the project folder is enough for VSCode to pick it up and prompt to reopen the folder in the container, including performing the setup steps. 

![Devcontainer preparing the environment](/assets/images/run-cuda-machine-learning-environment-in-docker/002.png)

The first run takes a while, VSCode is then connected to the container and the environment looks very familiar. The terminal shows that the virtual environment is active, and the path starts with `/workspaces/` which indicates that it's running inside the container. 
...
It should be enough to just clone the repo and open it in VSCode to get started. But make sure that the Nvidia Container Toolkit is installed on the host first, otherwise the container won't have access to the GPU.